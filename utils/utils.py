...
def distribute_population_stats(
    ...
    raster_array, valid_mask, out_transform = get_raster_clip_under_polygon(
        input_polygon, population_raster, transformer
    )
...
def calculate_demand_metrics_2sfca(
    ...
    try:
        ...
        park_centroid_raster = shapely_transform(
            park_transformer.transform, park_centroid
        )
        ...
        acres_per_1000_weighted = (
            (park_area_m2 / 4047) / (pop_weighted / 1000) if pop_weighted > 0 else None
        )
