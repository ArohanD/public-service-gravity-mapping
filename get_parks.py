...
def get_parks(point, radius, parks=DEFAULT_WEB_LAYER, out_fc=DEFAULT_OUT_FC):
    ...
    arcpy.AddMessage(
        f"Getting parks within a radius of {radius} meters from point: {x}, {y}"
    )
    ...
    try:
        if point_param is not None:
            ...
            if count > 0:
                ...
                arcpy.AddMessage(
                    f"Running as ArcGIS tool with point: {point}, radius: {radius}, parks: {parks}, out_fc: {out_fc}"
                )
            else:
